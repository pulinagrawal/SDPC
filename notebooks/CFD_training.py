...
model_name_prefix = 'CFD_2L_Wi_Feedback'

Use_tb = True #### Use to activate tensorboard monitoring
torch.discover_device = lambda: 'cuda:0'

## Database 
data_path = './DataSet/CFD/CFD'
train_percentage = .9
...
def setup_dataset(split='train'):
    transform = Compose([ToTensor(),
                        to_device(),
                        Resize((120, 171)),
                        whitening((120, 171),f_0=0.5),
                        LCN(kernel_size=11,sigma=0.5,rgb=True),
                        z_score(),
                        mask((120, 171))])
    ...
    mask_g = [gaussian_kernel((64,3,9,9), sigma=30),
              gaussian_kernel((128,64,9,9), sigma=30)
              ]
...
def setup_model():
    ...
    Net = Network(layer, input_size=(batch_size, 3, 120, 171))
    Loss = ML_Lasso(Net, l)

...
model_name_prefix = 'linear_search_overfit'
do_feedback = True

Use_tb = True #### Use to activate tensorboard monitoring
torch.discover_device = lambda: 'cpu'
...
def setup_dataset(split='train'):
    ...
    dataset = Subset(STL10(data_path, transform=transform, download=True, split='train'), range(4))
...
def train_STL_model(model, dataset, nb_epoch, Use_tb=True):
    ...
    path = Path('Savings') / 'STL' / (model_name +'.pkl')
    if Use_tb : 
        nrows = [8,8,8,8,8,8,8]
        writer = SummaryWriter(Path('Savings/Log') / model_name)
    ...
    path = Path('Savings') / (model_name +'.pkl')
...
if __name__ == '__main__':
    ...
    load_model = Path("Savings/STL_2L_Wi_Feedback[0.4,1.6]_b=1.pkl")
    load_model = None
    model = setup_model(load_model)
    if load_model is None:
        train_STL_model(model, train_dataset, nb_epoch, Use_tb=True)
    if load_model is not None:
        model_name_prefix = load_model.name.split('.')[0].split('[')[0]

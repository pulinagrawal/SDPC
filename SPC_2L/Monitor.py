...
class Monitor(object):
    def __init__(self, Net, writer, n_row=None, DicoL=True):
        ...
        self.total_activity = [torch.zeros(1,1,1,1).to(torch.discover_device())]*self.Net.nb_layers
        self.act = [torch.zeros(1,self.Net.layers[i].dico_shape[0],1,1).to(torch.discover_device()) for i in range(self.Net.nb_layers)]
       ...
       # self.x_bar = [np.arange(self.Net.layers[i].dico.data.size()[0]) + 1 for i in range(self.Net.nb_layers)]
        ...
        if DicoL:
            ...
            if torch.cuda.is_available():
                ...
                self.D_eff = [torch.randn(self.size_eff[i]).to(torch.discover_device()) for i in range(self.Net.nb_layers)]
                self.D_eff_m = [torch.randn(self.size_eff[i]).to(torch.discover_device()) for i in range(self.Net.nb_layers)]
            else:
